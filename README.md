...
- `scripts/verify_release_candidate.sh`：运行 beta 候选版本的本地验证 gate。
- `scripts/archive_testflight.sh`：生成 iOS archive，并可选上传到 TestFlight 处理流程。
- `scripts/package_macos_app.sh`：打包本地 macOS 预览 app。
- `docs/local_validation.md`：本地验证流程。
- `docs/testflight_release.md`：GitHub + TestFlight beta 分发流程。
...
2. 每次 beta 前运行 `scripts/verify_release_candidate.sh` 和手动 UI 检查。
3. 使用 tag 标记可测试版本，例如 `v0.2.0-beta.1`。
4. 使用 `scripts/archive_testflight.sh` 生成签名 archive，并在 Apple 账号配置完成后上传到 TestFlight 处理流程。
5. 将反馈转成 GitHub issue，按数值正确性、交互阻塞、性能退化和教学清晰度排序处理。

完整流程见 [docs/testflight_release.md](docs/testflight_release.md)。

## 当前优先级

- 接入 Apple Developer Team ID、bundle identifier 和 TestFlight tester group。
- 按 beta tag 生成第一版 TestFlight archive。
